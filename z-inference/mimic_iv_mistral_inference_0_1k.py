...
MISTRAL_API_KEY =  "glvnEmKjViyud8yPIEQjg1zf21pwY79j"
client = MistralClient(api_key=MISTRAL_API_KEY)
train_dataset = pd.read_csv('mimic-iv-sampled.csv')
# icd_9_df = pd.read_csv('/home/ashish/llama_inference/physionet.org/files/mimiciii/1.4/D_ICD_DIAGNOSES.csv')
all_symptoms = {}
# train_dataset = train_dataset.iloc[:1000]
from tqdm import tqdm
for i,row in tqdm(train_dataset.iterrows()) : 
    sys_prompt_template =  """
You are an AI assistant analyzing clinical discharge summary notes from the MIMIC IV dataset. Focus on these key sections:
...
- Keep each symptoms clear as mentioned in the discharge summary,  adhering to medical ontology standards.
...
"""
    ...
    all_symptoms[row.subject_id] = chat_response.choices[0].message.content



    try: 
        with open(f'mimic_iv_train_symptoms/{row.subject_id}.json', 'w') as f:
            json.dump(json.loads(chat_response.choices[0].message.content.strip('` \n').replace('json\n', '', 1)), f)
    except : 
        continue
