...
# Sample small list of classes for testing - replace with your classes
...
def compute_metrics(predictions, labels):
    ...
    f1_weighted = f1_score(labels, predictions, average='weighted', zero_division=1)
...
def train_and_evaluate():
    ...
    train_df = pd.merge(symptoms_train_df[['id','Symptoms']], train_df, on='id')
    val_df = pd.merge(symptoms_train_df[['id','Symptoms']], val_df, on='id')
    ...
    train_labels = mlb.transform(train_df['short_codes'])
    val_labels = mlb.transform(val_df['short_codes'])

    # Print label statistics
    print(f"Number of classes: {len(classes)}")
    print(f"Average labels per sample: {train_labels.sum()/len(train_labels):.2f}")

    # Create datasets and dataloaders
    train_dataset = SymptomICDDataset(
        train_df['Symptoms'],
        train_df['text'],
        train_labels,
        tokenizer
    ...
    # Load best model
    model.load_state_dict(torch.load("best_biobert_model.pt"))
    
    # Save model locally
    local_model_dir = "biolink_large_disease_classification"
    ...
    # Push to hub
    repo_id = "ashishkgpian/biolink_large_disease_classification"
    model.push_to_hub(repo_id, token='"YOUR_HF_TOKEN"')
    tokenizer.push_to_hub(repo_id, token='"YOUR_HF_TOKEN"')

    print(repo_id)
    print('biolink_finetune')
