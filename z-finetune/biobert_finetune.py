...
model.classifier = torch.nn.Linear(model.config.hidden_size, len(classes))  # replace 1185 with the exact number of classes
...
train_labels = mlb.transform(train_df['short_codes'])  # Fit and transform on training data
...
class SymptomICDDataset(Dataset):
    def __init__(self, symptoms, text, labels, tokenizer=tokenizer, max_length=512):
    ...
    def __getitem__(self, idx):
        ...
        text = self.text[idx]  # Assuming 'text' is another column in your dataset
        input_text = f"{symptom_text}\n{text}"  # Concatenate symptom text and text
...
# class SymptomICDDataset(Dataset):
#     def __init__(self, text, labels, tokenizer=tokenizer, max_length=512):
...
#         text = self.text[idx]  # Assuming 'text' is another column in your dataset
#         input_text = f"{text}"  # Concatenate symptom text and text
#         inputs = self.tokenizer(
#             input_text,
#             padding='max_length',
#             truncation=True,
...
train_dataset = SymptomICDDataset(train_df['Symptoms'] ,train_df['text'], train_labels)
val_dataset = SymptomICDDataset( val_df['Symptoms'], val_df['text'], val_labels)
test_dataset = SymptomICDDataset(test_df['Symptoms'], test_df['text'], test_labels)

# train_dataset = SymptomICDDataset( train_df['text'], train_labels)
# val_dataset = SymptomICDDataset( val_df['text'], val_labels)
# test_dataset = SymptomICDDataset(test_df['text'], test_labels)
...
local_model_dir = "biolink_large_disease_classification" # Local directory - it will be created if it doesn't exist

# Save LOCALLY first
model.save_pretrained(local_model_dir)
tokenizer.save_pretrained(local_model_dir)

repo_id = "ashishkgpian/biolink_large_disease_classification" # Correct repo ID
model.push_to_hub(repo_id, token = '"YOUR_HF_TOKEN"') # push model
tokenizer.push_to_hub(repo_id, token = '"YOUR_HF_TOKEN"') # push tokenizer

print('biobert_finetune')
